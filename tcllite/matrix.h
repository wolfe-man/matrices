...
// www.trumphurst.com/cpplibs/cpplibs.php
...
//  reallocation method
template< class T >
inline void matrix< T >::realloc (size_t row, size_t col)
{
...
template< class T >
inline void matrix< T >::SetSize (size_t row, size_t col)
{
    size_t i,j;
...
template< class T > inline T &matrix< T >::operator () (size_t row, size_t col)
{
    if (row >= _m->Row || col >= _m->Col)
        //throw exception( "matrix< T >::operator(): Index out of range!");
...
{
    if (row >= _m->Row || col >= _m->Col)
        throw std::runtime_error( "matrix< T >::operator(): Index out of range!");
    return _m->Val[row][col];
}
...
template< class T > inline bool operator==(const matrix< T >& m1, const matrix< T >& m2)
{
...
inline matrix< T > &matrix< T >::operator += (const matrix< T >& m)
{
    if (_m->Row != m._m->Row || _m->Col != m._m->Col)
        throw std::runtime_error( "matrix< T >::operator+= : Inconsistent matrix sizes in addition!");
    if (_m->Refcnt > 1) clone();
...
{
    if (_m->Row != m._m->Row || _m->Col != m._m->Col)
        throw std::runtime_error( "matrix< T >::operator-= : Inconsistent matrix sizes in subtraction!");
    if (_m->Refcnt > 1) clone();
    for (size_t i=0; i < m._m->Row; i++)
        for (size_t j=0; j < m._m->Col; j++)
            _m->Val[i][j] -= m._m->Val[i][j];
...
{
    if (_m->Col != m._m->Row)
        throw std::runtime_error( "matrix< T >::operator*= : Inconsistent matrix sizes in multiplication!");
...
template< class T >
inline matrix< T > &matrix< T >::operator^=(const size_t& pow)
{
    matrix< T > temp(*this);
...
template< class T >
inline matrix< T > operator*(const matrix< T >& m, const T& no)
{
    matrix< T > temp = m;
    temp *= no;
    return temp;
...
// binary scalar multiplication operator
template< class T >
inline matrix< T > operator*(const T& no, const matrix< T >& m)
{
    return (m * no);
}
...
template< class T >
inline matrix< T > operator^(const matrix< T >& m, const size_t& pow)
{
    matrix< T > temp = m;
    temp ^= pow;
    return temp;
...
// inversion function
template< class T >
inline matrix< T > matrix< T >::Inv()
{
    size_t i,j,k;
    T a1,a2,*rowptr;

    if (_m->Row != _m->Col)
        throw std::runtime_error( "matrix< T >::operator!: Inversion of a non-square matrix");
    ...
    for (k=0; k < _m->Row; k++)
    {
        int indx = pivot(k);
        if (indx == -1)
            throw std::runtime_error( "matrix< T >::operator!: Inversion of a singular matrix");
...
template< class T >
inline matrix< T > matrix< T >::Solve(const matrix< T >& v) const
{
    size_t i,j,k;
    T a1;

    if (!(_m->Row == _m->Col && _m->Col == v._m->Row))
        throw std::runtime_error( "matrix< T >::Solve():Inconsistent matrices!");
    ...
    for (k=0; k < _m->Row; k++)
    {
        ...
        if (indx == -1)
            throw std::runtime_error( "matrix< T >::Solve(): Singular matrix!");
...
template< class T >
inline void matrix< T >::Null(const size_t& row, const size_t& col)
{
    if (row != _m->Row || col != _m->Col)
	realloc( row,col);
...
template< class T >
inline void matrix< T >::Unit (const size_t& row)
{
...
template< class T >
inline int matrix< T >::pivot(size_t row)
{
    int k = int(row);
    double amax,temp;
...
inline T matrix< T >::Det() const
{
    ...
    if (_m->Row != _m->Col)
        throw std::runtime_error( "matrix< T >::Det(): Determinant a non-square matrix!");
...
template< class T >
inline T matrix< T >::Cofact(size_t row, size_t col)
{
    size_t i,i1,j,j1;

    if (_m->Row != _m->Col)
        //      throw exception( "matrix< T >::Cofact(): Cofactor of a non-square matrix!");

        if (row > _m->Row || col > _m->Col)
            throw std::runtime_error("matrix< T >::Cofact(): Index out of range!");
...
template< class T >
inline matrix< T > matrix< T >::Adj ()
{
    if (_m->Row != _m->Col)
        throw std::runtime_error( "matrix< T >::Adj(): Adjoin of a non-square matrix.");
...
template< class T >
inline bool matrix< T >::IsSingular ()
{
    if (_m->Row != _m->Col)
        return false;
    return (Det() == T(0));
...
template< class T >
inline bool matrix< T >::IsDiagonal()
{
    if (_m->Row != _m->Col)
        return false;
    for (size_t i=0; i < _m->Row; i++)
...
template< class T >
inline bool matrix< T >::IsSymmetric()
{
    if (_m->Row != _m->Col)
        return false;
    for (size_t i=0; i < _m->Row; i++)
...
template< class T >
inline bool matrix< T >::IsSkewSymmetric()
{
    if (_m->Row != _m->Col)
        return false;
    for (size_t i=0; i < _m->Row; i++)
...
template< class T >
inline bool matrix< T >::IsUpperTriangular()
{
    if (_m->Row != _m->Col)
        return false;
    for (size_t i=1; i < _m->Row; i++)
...
template< class T >
inline bool matrix< T >::IsLowerTriangular()
{
    if (_m->Row != _m->Col)
        return false;
